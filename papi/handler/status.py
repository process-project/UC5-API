...
class Status(Resource):
    ...
    def __init__(self, database: DB, ssh: Ssh, config: Mapping):
        ...
        self.ssh = ssh
        self.config = config
        self.jwt_enabled = self.config['JWT_REQUIRED']
    ...
    def __do(self, task_id: int):
        ...
        if status in ['queued']:
            pass
        elif status in ['running', 'executed', 'archiving', 'archived', 'aggregating']:
            status = 'running'
        elif status == 'failed':
            ...
            status_message = {"message": str(status_message),
                              ...
                              "result_webdav":
                                  "http://lobcder.process-project.eu:30530/lrzcluster/results/test"
                             }

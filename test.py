...
class JwtAuth(requests.auth.AuthBase):
    ...
    def __call__(self, r):
        ...
        key = ''
        with open("keys/private.pem", "r") as myfile:
            key = myfile.read()
...
if path == 'status':
    method = 'GET'
    url = f"http://{HOSTNAME}:{PORT}/{path}/{task_id}"
    resp = requests.get(url, auth=JwtAuth(), headers=headers)
elif path == 'submit':
    method = 'POST'
    url = f"http://{HOSTNAME}:{PORT}/{path}/"
    print(f"payload: {payload}")
    print(f"url: {url}")
    resp = requests.post(url, auth=JwtAuth(), data=payload, headers=headers)
else:
